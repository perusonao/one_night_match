...
未修正（要ユーザー判断、open questions 21・27番参照）:

- 12試合の自動シミュレーションで**0試合が決着しなかった**（安全弁として
  1501ターンまで実行）。キックアウトカード・返技エネルギーによる回避が
  ほぼ常に成功し、諦め（決着）が一度も発生しなかった。決着数値
  （`kickOutHpRate`／`kickOutThreshold`等）の調整、または試合時間切れ
  （仕様書14章）の実装が必要になる可能性が高い。
